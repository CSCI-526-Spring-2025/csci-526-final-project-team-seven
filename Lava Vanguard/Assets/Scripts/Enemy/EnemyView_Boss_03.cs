...
public class EnemyView_Boss_03 : EnemyView
{
    ...
    private IEnumerator BulletAttackRoutine()
    {
        while (true)
        {
            for (int i = 0; i < bulletCount; i++)
            {
                ...
                var bullet = Instantiate(bulletPrefab, transform).GetComponent<EnemyView_Boss_03_Bullet>();
                bullet.Init("Enemy_Boss_03_Bullet", transform.position, direction, bulletRotation);
